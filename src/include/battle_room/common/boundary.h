...
public:
     ...
     */
    virtual bool intersects(Boundary* other, Vector2D delta, radians angle);
     ...
     */
    virtual bool contains(Vector2D delta) = 0;

protected:

    virtual Projection1D projectOnto(Vector2D axis) = 0;
    virtual std::vector<Vector2D> getSideNormals() = 0;
